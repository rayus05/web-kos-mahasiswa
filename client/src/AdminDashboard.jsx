...
function AdminDashboard() {
  ...
  const fetchStats = async () => {
    try {
      const res = await axios.get('https://edukost1.vercel.app/api/stats');
      setStats(res.data);
  ...
  const fetchUsers = async () => {
    try {
      const res = await axios.get('https://edukost1.vercel.app/api/users');
  ...
  const handleDeleteUser = async (id, usernameUser) => {
    if (window.confirm(`Yakin ingin memblokir/menghapus user "${usernameUser}"?`)) {
      try {
        await axios.delete(`https://edukost1.vercel.app/api/users/${id}`);
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get('https://edukost1.vercel.app/api/admin/kos');
  ...
  const handleDelete = async (id, nama) => {
    if (window.confirm(`Yakin hapus "${nama}"?`)) {
      try {
        await axios.delete(`https://edukost1.vercel.app/api/kos/${id}`);
        fetchData();
      } catch (error) {
